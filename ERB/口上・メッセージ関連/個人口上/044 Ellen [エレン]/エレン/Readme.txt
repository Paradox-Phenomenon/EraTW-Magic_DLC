eraTW4.571用 エレン口上セット Ver0.67
※バランス調整に不慣れなため、少々バランスが悪いかもしれません。
　「バランスおかしいだろ」、と思ったらガンガン突っ込んでください
―――――――――――――――――――――――――――――――――――――――――――――――

１.これってどんな口上？
　ふわふわでぱちぱちでぽわわんなエレン、ようは普通のエレン口上です。もちろん純愛仕様なのよーっ！
　あなたが男であること前提で組んでおります。同姓仕様は…未定です
　大体日常系、うふふはなぜか押し倒し時と押し倒され時と終了時だけあったり
　仕事も追加されてます、はたらきものだからね

２.仕事に関して
　毎月十五、三十日の14時～19時に人里の広場で魔法屋を出張営業します
　TCVARを利用して客数を設定しています。客数に応じて文章が分岐したり、手伝う内容が変化したりします
　それ以外の日にも、低確率ですが古木の大樹かキノコ群生地で採集を行います
　こちらは魔法の森を拠点にしているときのみ発生します

３.イベントについて

　・エレンに弟子入り
　　あなたの教養がある程度あり、エレンとけっこう親しい時に会話するとたまに発生します
　　勉強、戦闘訓練で魔法の修行ができるようになります

　・魔女の薬
　　あなたの体力、もしくは気力が低く、エレンとある程度親しいと会話した際にたまに発生します
　　あなたの体力と気力が大きく回復します。デメリット無しですが発生率は低め

これ以外にもいくつかイベントがあります

４.これからどうするの？
　会話パターンを増やしたり、特殊会話を増やしたり、イベントを増やしたりとか、みたいな？
　食事口上は大体出来たので次は不機嫌時の分岐とか場所ごとの会話を増やしたりとか、ミニゲーム的なものも足したいなあ……　あれ？eraってこんなゲームだったっけ？
　弟子入り後イベもちょろちょろと追加したいところ
　
５.参考にさせて頂いた口上や助言を貰った人など
　・eratohoYM 汎用逆口上ジェネレーターv2.00proto
　　口上作成の基礎を学ばせていただきました　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　何でこれで基礎を学んだかって？いや、まあ、その
　・eratohoYM Mk-3改用霊夢口上｢おしゃぶり霊夢口上ｖ5.01｣
　　擬音や地の文等を参考にさせていただきました
　・eraTheWorld proto3.95用 Amazing! 多々良小傘口上
　　当口上作成において会話の構文や、TCVAR・FIRSTTIMEの使い方などとても参考にさせていただきました
　・eraTheWorld proto4.00用 影狼口上Ver.α
　　食事周りの構文を参考にさせていただきました
　・eraTW さとりん口上の人
　　ずっと気が付いていなかった構文のミスをわざわざ探して指摘してくださった素晴らしいお方。頭が上がりません
　この場を借りてお礼申し上げます！

６.加筆修正などに関して
　口上フォルダ内のライセンスをご確認下さい

７.更新履歴
　2025/08/14　(※別作者)
    eraTW 4.974の更新作業途中に、本口上の一部を修正しました

　2018/1/7 Ver0.67
　　口上を追加
　　CFLAG:子供の性別を「CHILD_SEX」に移行、そのため口上オプションの「子供の性別」を削除
　　育児口上が呼び出された際にエラー落ちするのを修正

　2017/11/15 Ver0.65
　　口上を追加＆変更＆修正
　　まったく使ってなかった自慰系コマンドのファイルを削除
　　育児部分の本体取り込みにつき、口上オプションの子供の演出を「子供の性別」に変更

　2017/9/8 Ver0.61
　　口上を追加＆変更＆修正
　　子供の演出において子供の性別を選択できるように
　　└口上オプション同様エレンと願掛けで選択可能

　2017/9/5 Ver0.60
　　今更ながらライセンスを同梱
　　口上を追加＆変更＆修正
　　口上オプションとして「健全モード」と「子供の演出」を追加
　　└変更はエレンと一緒に願掛けすることで可能です
　　恋慕獲得後、遭遇時に毎回恥じらい獲得のテロップが出てたのを修正
　　魔法の披露の難易度を一部変更
　　うふふ時のお香イベントを育児中は発生しないように変更

　2017/8/14 Ver0.55
　　口上を追加＆変更＆修正
　　弟子入り後のイベントが途中から進まなくなっていたのを修正。弟子入り済みの人は勉強or訓練する前にアプデしてください

　2016/11/16 Ver0.5
　　口上を追加＆変更＆修正
　　仕事に魔法の森の採集を追加、拠点が魔法の森かつ出張を行わない日に天気が良いと低確率で発生します
　　└それに伴い出張を行う日が15日と30日に変更
　　各ペナルティの大幅緩和
　　非恋慕非恋人でも時姦刻印+アルファで押し倒せるように

　2016/02/28 Ver0.4
　　口上を追加＆変更＆修正
　　現在の仕様に合わせて各箇所の変更（出掛けるなど）
　　アップデート時に一度だけNAMEの変更をできるように
　　行動範囲を拡張。詳しくは出現地域メモを
　　自宅位置を魔法の森入り口に設定

　2015/08/19 Ver0.31
　　口上を追加＆変更＆修正

　2015/06/18 Ver0.3
　　口上を追加＆変更＆修正
　　CSVを若干修正
　　AAを追加＆表示条件を変更

　2015/03/31 Ver0.21
　　口上を追加＆変更＆修正
　　口上イベントの追加、変更にあわせてファイルに追記

　2015/03/19 Ver0.2
　　口上を追加＆変更＆修正
　　CSVのキャラ紹介を加筆
　　AAの表示条件を修正＆変更

　2015/03/06 Ver0.18
　　イベントでエレンのバストサイズが一時的に上昇した後元のサイズに戻らないのを修正
　　既にイベントを起こしている人用にエレンに遭遇すればバストサイズが戻るよう修正用の構文を追加
　　（巨乳→絶壁、爆乳→貧乳になります、一回のみ）
　　口上を追加＆変更、主に食事系

　2015/03/01 Ver0.11
　　口上を追加＆変更＆修正、日常系は大体埋まりました。次はデート系か…
　　怒り時も笑顔なのは変なので怒り時のAAを追加。下手でごめんなさい
　　お仕事中の処理を若干変更、キャラを切り替えるたびにセリフが出ないようにしました。ついでに仕事終了時のデートをキャンセル可能に　　
　　MOVEMENT2を修正、…というよりさとりん口上の物に変更しました。こちらのほうがスマートですし、自分が作ったのだとうふふ帰宅でもデート帰宅口上になるので
　　さとりん口上の作者様にこの場を借りてお礼申し上げます

　2015/02/28 Ver0.1
　　とりあえずリリース

８.あとがき
　前略、作者です
　口上作成歴が３年近いのに何かと作りが変な口上ですが、楽しんでくれると幸いです
　この口上を通してエレンを好きになってくれた人が居るみたいでとても嬉しいです。エレン可愛いよエレン
　ええと、まあそれだけです、はい
　うじゃうじゃ